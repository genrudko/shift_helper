...
Public Function SH_T(ByVal key As String) As String
    Select Case key
        Case "ERR_JOURNAL": SH_T = "Active workbook is not a Shift-Helper journal."
        ...
        Case "CAL_TITLE": SH_T = SH_U("00530068006900660074002D00480065006C007000650072002020140020043A0430043B0435043D043404300440044C")
        Case "CAL_TODAY": SH_T = SH_U("042104350433043E0434043D044F")
        Case "CAL_CANCEL": SH_T = SH_U("041E0442043C0435043D0430")
...
Public Function SH_ReportDate(ByVal wb As Workbook) As Date
    ...
    value = wb.Worksheets(SH_PrepSheetName()).Range(SH_ReportDateCell()).Value
    If Not IsDate(value) Then Err.Raise vbObjectError + 513, , "Invalid report date in B3."
    SH_ReportDate = CDate(value)
End Function

Public Function SH_ReportOffset(ByVal wb As Workbook) As Double
    Dim value As Variant
    value = wb.Worksheets(SH_PrepSheetName()).Range(SH_ReportOffsetCell()).Value2
...
Public Sub SH_SaveSettingValue(ByVal wb As Workbook, ByVal labelText As String, ByVal value As String)
    ...
    Next r
    If targetRow = 0 Then targetRow = SH_LastRow(ws, 13) + 1: If targetRow < 1 Then targetRow = 1
