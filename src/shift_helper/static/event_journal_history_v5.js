...
(() => {
    ...
    function applyCellChanges(changes, side) {
        ...
        try {
            for (const change of changes) {
                ...
                valueCache.set(cacheKey(change.rowKey, change.field), value);
        ...
        try {
            selected.forEach((row, rowIndex) => {
                editableFields.forEach((field, fieldIndex) => {
                    const value = matrix[rowIndex % matrix.length]?.[
                        fieldIndex % matrix[0].length
                    ] ?? "";
                    valueCache.set(cacheKey(row.getData()._rowKey, field), value);
    ...
    table.on("rowUpdated", (row) => {
        const data = row.getData();
        const previousKey = [...valueCache.keys()]
            .map((key) => key.split("\u0000")[0])
            .find((key) => key.startsWith("draft-") && !rowByKey(key));
        if (previousKey && previousKey !== data._rowKey) {
            remapKey(previousKey, data._rowKey);
        }
